...
import { Agent, type IncomingMessage } from "node:http";
...
import type { ByondProxyConfig } from "./config_types";
import { queued_invoke } from "./promise_queue";

const config: Readonly<ByondProxyConfig> = await (async () => {
	const config_paths = [
		fileURLToPath(new URL("../config.json", import.meta.url)),
		fileURLToPath(
			new URL("../../config/webclient-proxy.json", import.meta.url),
		),
	...
	if (config.trusted_proxies) {
		const list = [];
		for (const proxy of config.trusted_proxies) {
...
if (config.webclient_origin == config.browse_origin) {
	console.warn(
		"WARNING - webclient_origin and browse_origin are both " +
			config.webclient_origin,
	);
	console.warn(
		"Please put those on separate origins to prevent against potential browse() vulnerabilities",
	);
}

const openid_client = (async () => {
	const info = config.openid_info;
	if (!info) return undefined;
	const issuer = await Issuer.discover(info.origin);
	const client = new issuer.Client(
		Object.assign(info.client_data, {
...
wss.on("headers", (headers, req) => {
	const toinsert = wss_header_map.get(req);
	if (toinsert) {
...
const despam_map = new Map<string, (cb: () => Promise<any>) => Promise<any>>();
function despammedInvoke<T>(
...
function getProxyAddress(request: IncomingMessage, socket: Socket) {
	...
	if (config.use_cf_connected_ip) {
		...
		const proxies = proxy_header?.split(",");
		while (proxies && proxies.length) {
			if (address && proxy_matches(address)) {
				address = proxies.pop();
			} else {
				console.log(
					"Untrusted proxy " + address + " trying to be " + proxies.join(", "),
				);
				break;
			}
		}
...
function getProxyAgent(request: IncomingMessage, socket: Socket, url?: string) {
	...
	const [byond_host, byond_port] = config.byond_addr.split(":");
	const address = getProxyAddress(request, socket);

	let agent: Agent | undefined;
	if (config.byond_proxy_addr) {
		const addr = config.byond_proxy_addr;
		agent = new Agent();
		// @ts-expect-error
		agent.createConnection = (
			...
			const [dstaddr, dstport] = addr.split(":");
			const socket = connect(+dstport, dstaddr);
			const port = 32768 + Math.floor(Math.random() * 28232);
			socket.write(
				`PROXY TCP4 ${address} 255.255.255.255 ${port} ${byond_port}\r\n`,
			);
			return socket;
		};
	}
...
async function upgrade_request(
	request: IncomingMessage,
	socket: Socket,
	head: Buffer,
) {
	try {
		...
		const login_token = `wcp:${(await promisify(randomBytes)(32)).toString("hex")}`;
		const user_info_str = `${encodeURIComponent(user_info.key)};gender=${user_info.gender}`;
		const [byond_host, byond_port] = config.byond_addr.split(":");
		await sendTopic({
			// Tell webclient_patches to expect us
			host: byond_host,
			port: +byond_port,
		...
		const byond_connection = await new Promise<WebSocket>((resolve, reject) => {
			const ws = new WebSocket("ws://" + config.byond_addr, {
				agent: getProxyAgent(request, socket),
			});
		...
		byond_connection.send(
			Buffer.concat([
				Buffer.from([0, 1, 0, 0]),
				Buffer.from(browserfill, "utf8"),
				Buffer.from([0]),
			]),
		);
		const messages: Buffer[] = [];
		const interim_handler = (data: Buffer, isBinary: boolean) => {
			if (!isBinary) return;
			messages.push(data);
		};
		...
		wss.handleUpgrade(request, socket, head, (client: WebSocket) => {
			...
			byond_connection.on(
				"message",
				(final_handler = (data: Buffer, isBinary: boolean) => {
					if (!isBinary) return;
					const msgtype = data.readUint16BE(0);
					if (msgtype == 243) {
						let textlen = 0;
						let end = 2;
						textlen = data.readUInt16LE(end);
						end += 2;
						if (textlen == 0xffff) {
							textlen = data.readUInt32LE(end);
							end += 4;
						}
						const browse_url = data
							.subarray(end, end + textlen)
							.toString("utf8");
						end += textlen;
						const suffix = data.subarray(end);
						...
						domain_map.set(
							new_path,
							`http://${config.byond_addr}/${browse_url}`,
						);
						setTimeout(() => {
							domain_map.delete(new_path);
						}, 30000);
						const new_browse_url = `${config.browse_origin}/browse/${domain_string}/${new_path}`;
						const encoded = Buffer.from(new_browse_url);
						client.send(
							Buffer.concat([
								Buffer.from([
									0,
						...
						if (end < data.length) {
							const ctrlstart = end;
							while (end < data.length && data[end]) end++;
							const ctrl = data.subarray(ctrlstart, end).toString("utf8");
							if (ctrl == "byond-wcp:browse_rsc") {
								const eq_index = msg.indexOf("=");
								if (eq_index < 0) return;
								const resource = msg.substring(0, eq_index);
								const targetpath = msg.substring(eq_index + 1);
								domain_map.set(
									targetpath,
									`http://${config.byond_addr}/${resource}`,
								);
								return;
							}
						}
			...
			byond_connection.on("close", () => {
				browse_domains.delete(domain_string);
				client.close();
			});
			client.on("close", () => {
				browse_domains.delete(domain_string);
				byond_connection.close();
			});
			messages.length = 0;
		});
	} catch (e) {
...
app.use("/browse/:userHash", async (req, res, next) => {
	try {
		...
		const domain = browse_domains.get(req.params.userHash);
		if (!domain) {
			res.status(404);
			res.end("Not Found");
			return;
		...
		const theUrl = url;
		const fetch_res = await despammedInvoke(req, req.socket, () =>
			retryFetch(theUrl, undefined, getProxyAgent(req, req.socket, url)),
		);
		...
		res.setHeader(
			...
			"default-src 'self' 'unsafe-inline' 'unsafe-eval'; " +
				...
				"base-uri 'self' https: http: data: blob:;",
		);
		...
		for (const header of [
			...
			const val = fetch_res.headers.get(header);
			if (!val) continue;
...
app.use("/cache/", async (req, res, next) => {
	try {
		...
		if (path.startsWith("/")) path = path.substring(1);
		path = path.split("?")[0];
		const target_url = `http://${config.byond_addr}/cache/${path}`;
		const fetch_res = await despammedInvoke(req, req.socket, () =>
			retryFetch(target_url, undefined, getProxyAgent(req, req.socket)),
		);
		for (const header of [
			"cache-control",
			"content-disposition",
...
function parse_cookies(cookies: string | undefined) {
	return new Map(
		(cookies ?? "").split("; ").map((a) => a.split("=") as [string, string]),
	);
}
...
app.get("/logout", (req, res) => {
	...
	const cookies = parse_cookies(req.header("cookie"));
	const user_info = http_tokens.get(cookies.get(COOKIE_HTTP_TOKEN)!);
	if (user_info) {
...
app.get("/play", async (req, res) => {
	try {
		...
		if (user_info) {
			...
			const ws_token = (await promisify(randomBytes)(32)).toString("base64url");
			ws_tokens.set(ws_token, user_info);
			setTimeout(() => ws_tokens.delete(ws_token), LOGIN_EXPIRY);
			res.cookie(COOKIE_WS_TOKEN, ws_token, {
				httpOnly: true,
				maxAge: WS_TOKEN_EXPIRY,
				sameSite: true,
				secure: config.webclient_origin.startsWith("https://"),
			});
...
app.use(
	"/res",
	express.static(fileURLToPath(new URL("./res", import.meta.url))),
);
app.get("/", async (req, res, next) => {
	try {
		...
		if (ws_tokens.has(cookies.get(COOKIE_WS_TOKEN)!) || !config.openid_info) {
			res.sendFile(
				fileURLToPath(new URL("./res/index.html", import.meta.url)),
				(err) => {
					if (err) next(err);
				},
			);
		...
		if (!user_info) {
			...
			if ("code" in req.query) {
				...
				const http_token = (await promisify(randomBytes)(32)).toString(
					"base64url",
				);
				http_tokens.set(
					http_token,
