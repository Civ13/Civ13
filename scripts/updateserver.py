...
if not c1 and not c2 and not c3 and not c4:
	...
	subprocess.call(['/bin/bash', '-i', '-c', "restart-inactive-server"])
...
#lastMapSwapStatus = open("/home/customer/lastMapSwapStatus.txt", "w").read()
#if lastMapSwapStatus != "":
#	os.system('cd && sudo python3 mapswap.py {}'.format(lastMapSwapStatus))
#	print("Reset server's mapswap to {}".format(lastMapSwapStatus))
