		...
		if (client.status_tabs && statpanel("Status") && ticker)
			...
			if (map && !map.civilizations)
				...
				for (var/faction in map.faction_organization)
					if (!list(BRITISH, PIRATES, INDIANS, PORTUGUESE, SPANISH, FRENCH, DUTCH, ITALIAN, CIVILIAN, ROMAN, GREEK, ARAB, JAPANESE, RUSSIAN, GERMAN, AMERICAN, FILIPINO, CHECHEN, CHINESE, FINNISH, NORWEGIAN, SWEDISH, DANISH, VIETNAMESE, POLISH, REDFACTION, BLUEFACTION).Find(faction))
						continue
