...
/obj/map_metadata/iwojima/job_enabled_specialcheck(var/datum/job/J)
	...
	if (J.is_ww2 == TRUE || J.is_navy == TRUE)
		. = TRUE
	else if (J.is_tanker == TRUE || J.is_prison == TRUE || istype(J, /datum/job/japanese/ija_sergeant_tanker) || istype(J, /datum/job/japanese/ija_ww2_tanker) || istype(J, /datum/job/american/soldier_ww2_filipino))
		. = FALSE
	else
		. = FALSE
