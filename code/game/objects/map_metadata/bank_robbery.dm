...
/obj/map_metadata/bank_robbery/update_win_condition()
	...
	// German major
		...
		if (!win_condition.check(typesof(roundend_condition_sides[roundend_condition_sides[1]]), roundend_condition_sides[2], roundend_condition_sides[1], 1.33))
			if (last_win_condition != win_condition.hash)
				current_win_condition = "The Police controls the Vault! They will win in {time} minutes."
	...
	// Soviet minor
		...
		if (current_win_condition != no_winner && current_winner && current_loser)
			world << "<font size = 3>The Robbers have pushed the Police out of the Vault!</font>"
			current_winner = null
			current_loser = null
		next_win = -1
