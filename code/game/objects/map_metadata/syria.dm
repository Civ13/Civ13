/obj/map_metadata/syria
	...
	battle_name = "Syrian War fight over a town"
	mission_start_message = "<font size=4>The <b>Syrian Salvation army</b> and the <b>Syrian Goverment forces</b> are battling for the control of  a town in syria! Each side will win if they manage to hold the Goverment Building for <b>10 minutes</b>.<br>The battle will start in <b>5 minutes</b>.</font>"
	faction1 = ARAB
	faction2 = AMERICAN
	songs = list(
...
/obj/map_metadata/syria/cross_message(faction)
	if (faction == AMERICAN)
		return "<font size = 4>The Syrian Salvation Army may now cross the invisible wall!</font>"
	else if (faction == ARAB)
		return "<font size = 4>The Syrian Goverment Forces may now cross the invisible wall!</font>"
	else
		return ""

/obj/map_metadata/syria/reverse_cross_message(faction)
	if (faction == AMERICAN)
		return "<span class = 'userdanger'>The Syrian Salvation Army may no longer cross the invisible wall!</span>"
	else if (faction == ARAB)
		return "<span class = 'userdanger'>The Syrian Goverment Forces may no longer cross the invisible wall!</span>"
	else
		return ""
	...
	// German major
		...
		if (!win_condition.check(typesof(roundend_condition_sides[roundend_condition_sides[1]]), roundend_condition_sides[2], roundend_condition_sides[1], 1.33))
			if (last_win_condition != win_condition.hash)
				current_win_condition = "The [roundend_condition_def2army(roundend_condition_sides[1][1])] has captured the Goverment Building! They will win in {time} minute{s}."
