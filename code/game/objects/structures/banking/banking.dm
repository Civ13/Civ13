...
/obj/structure/banking/atm/do_html(mob/living/human/user)
	var/fullpage = {"
			<!DOCTYPE html>
			<html>
...
/datum/banking_program/atm/proc/do_html(mob/living/human/user)
	if (mainmenu == "---")
	...
	if (href_list["atm"])
		if (href_list["atm"] == 2)
			if (user.bank_hold)
				...
				playsound(origin.loc, 'sound/machines/atm/access_denied.ogg')
				depositing = FALSE
				return
			else
				mainbody += "Please insert the cash into the ATM. Only paper banknotes are accepted"
				playsound(origin.loc, 'sound/machines/atm/atm_deposit_open_1.ogg', 100, TRUE)
				depositing = TRUE
				return
		...
		if(findtext(href_list["atm"], "wdollar"))
			if (user.bank_hold)
				...
				if ((user.bank_dollar - w_amount) < 0)
					...
					playsound(origin.loc, "sound/machines/atm_withdraw.ogg")
					sleep(32)
					user.bank_dollar -= w_amount
					new/obj/item/stack/money/dollar(origin.loc, round(w_amount))
		...
		if(findtext(href_list["atm"], "wdrubles"))
			if (user.bank_hold)
				...
				if ((user.bank_dollar - w_amount) < 0)
					...
					new/obj/item/stack/money/rubles(origin.loc, round(w_amount))
					mainbody = "<font color ='green'><b>SUCCESS! Money withdrawn</b></font> <br>"
					mainbody += "<b>Have a nice day!</b>"
					sleep(0.5)
					do_html(user)
...
/obj/structure/banking/computer/atm/attack_hand(var/obj/item/O as obj, var/mob/living/human/user as mob)
	if (depositing == FALSE)
		return
	else
