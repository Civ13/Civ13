	...
	if (istype(W, /obj/item/weapon/wrench))
		...
		if (istype(W, /obj/item/stack/money))
			...
			if (accepted_currency == "standard" && istype(W, /obj/item/stack/money/fiat))
				...
				user << "<span class='warning'>This stall only accepts [fiat.currency_list[accepted_currency][1]] as operating funds. Use standard coins to stock inventory.</span>"
				return

			var/obj/item/stack/money/M = W
			if (accepted_currency == "standard")
				moneyin += M.value * M.amount
				user << "You add the money to the stall. It now has [moneyin] in standard coin value to fulfill orders."
			else
				moneyin += M.amount
				user << "You add the money to the stall. It now has [moneyin] [fiat.currency_list[accepted_currency][1]] to fulfill orders."
			qdel(W)
			return
		else if (!ordertype)
		...
		if (W.type == ordertype || (orderfiat_id == null && ordertype == /obj/item/stack/money && istype(W, /obj/item/stack/money) && !istype(W, /obj/item/stack/money/fiat)))
			...
			if (istype(W, /obj/item/stack))
				...
				moneyin -= payment
				orderamount -= buy_qty
				user << "<span class='notice'>You sell [buy_qty] [ordername].</span>"
				return
			else
				if (orderamount < 1)
					user << "<span class='warning'>The [src] is not currently buying.</span>"
...
/obj/structure/supplier/attack_hand(mob/living/human/H as mob)
	if (find_company_member(H,owner))
		...
		if (choice == "Cancel")
			H << "<big>This company is currently buying [orderamount] of [ordername] for [orderprice] [currency_name] per unit. It has [moneyin] [currency_name] inside.</big>"
			return
			...
			if (choice3 < 0)
				...
				orderamount = choice3
			return
	H << "<big>This company is currently buying [orderamount] of [ordername] for [orderprice] [accepted_currency == "standard" ? "sc" : fiat.currency_list[accepted_currency][1]] per unit.</big>"
	return
