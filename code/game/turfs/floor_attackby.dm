...
/turf/proc/mining_proc(var/mob/living/human/H)
	if (!H || !src)
		return
	var/turf/T = get_turf(src)
...
*/
	...
	if(istype(T, /turf/floor/dirt/underground/sandy) || (area_above && area_above.climate == "desert"))
	...
	else if (prob(20) && (map.ID != MAP_NOMADS_PANGEA && map.ID != MAP_NOMADS_CONTINENTAL && map.ID != MAP_NOMADS_NEW_WORLD && map.ID != MAP_NOMADS_MEDITERRANEAN) && map.ordinal_age >=1)
		...
		H.adaptStat("strength", 1)
		return
	else if (prob(20) && (area_above && area_above.climate == "temperate" || area_above && area_above.climate == "semiarid") && (map.ID == MAP_NOMADS_PANGEA && map.ID == MAP_NOMADS_CONTINENTAL && map.ID == MAP_NOMADS_NEW_WORLD && map.ID == MAP_NOMADS_MEDITERRANEAN) && map.ordinal_age >=1)
		var/obj/item/stack/material/marble/mineral = new/obj/item/stack/material/marble(src)
		mineral.amount = rand(8,12)
		if (istype(get_area(src), /area/caribbean/void/caves/special))
			mineral.amount *= 2
