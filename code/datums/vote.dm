			...
			switch(vote_type)
				...
				if ("custom")
					...
					if (!choices.len)
						...
						choices.Add("No")
				if ("gamemode")
					var/list/options = list("Classic (Stone Age Start)", "Chad Mode", "Auto-Research Mode", "Resource-Based Research", "Bronze Age (No Research)", "Medieval (No Research)", "Imperial Age (No Research)", "Industrial Age (No Reesearch)")
					if (!default)
