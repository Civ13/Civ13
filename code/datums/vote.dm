...
/datum/vote
	...
	proc/autogamemode()
		if (map.ID == MAP_NOMADS_EXTENDED || map.ID == MAP_CIVILIZATIONS || map.ID == MAP_TRIBES)
			return
			...
			switch(vote_type)
				...
				if ("custom")
					...
					if (!choices.len)
						...
						choices.Add("No")
				if ("gamemode")
					var/list/options = list("Classic (Stone Age Start)", "Chad Mode", "Auto-Research Mode", "Resource-Based Research", "Bronze Age (No Research)", "Medieval (No Research)", "Imperial Age (No Research)", "Industrial Age (No Research)", "Modern Age (No Research)", "WW2 Age (No Research)")
					if (!default)
