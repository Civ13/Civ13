		...
		if (.)
			switch(mode)
				...
				if ("map")
					...
					var/F = file("[config.masterdir]/1713/discord/gamedata.txt")
					if (fexists("[config.masterdir]/1713/discord/gamedata.txt"))
						fdel(F)
					var/string1 = "map:[processes.mapswap.next_map_title]\n"
					var/string2 = "players:[clients.len]\n"
					text2file("[string1][string2]","[config.masterdir]/1713/discord/gamedata.txt")
				if ("gamemode")
			...
			switch(vote_type)
				...
				if ("custom")
					...
					if (!choices.len)
						...
						choices.Add("No")
				if ("gamemode")
					var/list/options = list("Classic (Stone Age Start)", "Chad Mode", "Auto-Research Mode", "Resource-Based Research", "Bronze Age (No Research)", "Medieval (No Research)", "Imperial Age (No Research)", "Industrial Age (No Research)")
					if (!default)
