...
/mob/living/carbon/human/proc/checkrace()
	if(!orc && !ant && !wolfman && !lizard && !gorillaman && !crab)
		return FALSE
	else
		return TRUE
/mob/living/carbon/human/rad_act()
	..()
	if(radiation >= 100) //If you are super irradiated, and somehow still alive.
		if (prob(15))
			if (prob(15))
			...
			radiation -= radiation/4 //Reduce radiation because you ain't resistant.
	if(radiation >= 150) //Corpse gotta be pretty bad tbh.
		if(stat == DEAD) //if dead.
			if (prob(20)) //decent chance.
				var/i
