		...
		if (H.original_job)
			if (istype(item, /obj/item/weapon/reagent_containers/food/drinks/bottle))
				...
				if (B.rag && B.rag.on_fire)
					var/nothrow = FALSE
					if (map && !map.pirates_can_cross_blocks() && list(CIVILIAN, PIRATES, INDIANS).Find(H.original_job.base_type_flag()))
						nothrow = TRUE
					else if (map && !map.british_can_cross_blocks() && list(BRITISH, FRENCH, PORTUGUESE, SPANISH).Find(H.original_job.base_type_flag()))
						nothrow = TRUE
					if (nothrow)
