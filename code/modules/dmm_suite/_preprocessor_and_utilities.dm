...
atom/New(turf/newLoc)
    if(isturf(newLoc) && newLoc.dmm_preloader)
...
dmm_suite/preloader
	parent_type = /datum
	var/list/attributes
dmm_suite/preloader/New(turf/loadLocation, list/_attributes)
	loadLocation.dmm_preloader = src
	attributes = _attributes
	. = ..()
dmm_suite/preloader/proc/load(atom/newAtom)
