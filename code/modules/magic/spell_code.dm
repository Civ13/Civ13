/spell
	...
	*/
	...
	var/overlay = 0
	var/overlay_icon = 'icons/obj/magic_projectiles.dmi'
...
/spell/proc/perform(mob/user = usr, skipcharge = 0) //if recharge is started is important for the trigger spells
	...
	if(!check_valid_targets(targets))
		return
	var/time = 0
	msg_admin_attack("[user.ckey] attempted to cast the spell [name]")
	do
		time++
		...
		if(cast_check(1,user, targets)) //we check again, otherwise you can choose a target and then wait for when you are no longer able to cast (I.E. Incapacitated) to use it.
			...
			break
	while(time != number_of_channels && do_after(user, time_between_channels, incapacitation_flags = INCAPACITATION_KNOCKOUT|INCAPACITATION_FORCELYING|INCAPACITATION_STUNNED))
...
/spell/proc/cast_check(skipcharge = 0,mob/user = usr, var/list/targets) //checks if the spell can be cast based on its settings; skipcharge is used when an additional cast_check is called inside the spell
	...
	if((spell_flags & Z2NOCAST)) //Certain spells are not allowed on the centcomm zlevel
		return 0
...
/spell/proc/invocation(mob/user = usr, var/list/targets) //spelling the spell out and setting it on recharge/reducing charges amount
	...
	switch(invocation_type)
		...
		if(SpI_EMOTE)
			user.custom_emote(1, invocation)
	...
	if (isliving(src))
		...
		if(mind)
			for (var/spell/S in mind.learned_spells)
				dat += {"
				<a href='byond://?src=\ref[src];spellcast=[S]'>
...
/mob/Topic(href, href_list[])
	..()
	if (href_list["spellcast"])
		msg_admin_attack("[href_list["spellcast"]]")
		var/spell/spell = href_list["spellcast"]
		spell.perform(usr)
		return
