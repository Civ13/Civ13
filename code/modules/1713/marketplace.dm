/obj/structure/marketplace
	name = "DEEPNET terminal"
	desc = "When what's sold in stores is not good enough."
...
/obj/structure/marketplace/attack_hand(var/mob/living/human/user as mob)
	var/choice =  WWinput(user, "Do you want to buy something, or change a order you placed on the market?", "DEEPNET", "Cancel", list("Buy", "Change Orders", "Withdraw Money","Cancel"))
	if (choice == "Cancel")
		return
		...
		if (map.marketplaceaccounts[user.civilization])
			...
			if (accmoney <= 0)
				...
				return
			var/choicew =  WWinput(user, "You have [accmoney/4] dollars in your company's account.", "DEEPNET", "Cancel", list("Withdraw","Cancel"))
			if (choicew == "Cancel")
				return
			...
			user << "You company hasn't sold anything yet! The account is empty."
			return
		...
		if (choice2 == "Cancel")
			...
			for (var/list/k in currlist)
				if (choice2 == "[k[2]]")
					var/cost = (map.globalmarketplace[k[1]][4])
					if (!istype(user.l_hand, /obj/item/stack/money) && !istype(user.r_hand, /obj/item/stack/money))
						...
						if (mstack.value*mstack.amount >= cost)
							...
							if (mstack.amount<= 0)
								qdel(mstack)
							var/obj/BO = map.globalmarketplace[k[1]][2]
							if (BO)
								BO.forceMove(get_turf(src))
							map.globalmarketplace[k[1]][7] = 0
							user << "You fulfill the order."
							var/seller = map.globalmarketplace[k[1]][1]
							map.marketplaceaccounts[seller] += cost
							map.globalmarketplace -= map.globalmarketplace[k[1]]
							return
						else
			...
			for (var/list/k in currlist)
				if (choice2 == "[k[2]]")
					...
					if (choice3 == "Cancel")
						...
						var/newprice = input(user, "What shall the new price be, in dollars?") as num|null
						if (!isnum(newprice))
							return
						if (newprice <= 0)
							return
						map.globalmarketplace[k][4] = newprice/4
						user << "You update the listing's price."
						return
	else
		return
/obj/structure/marketplace/attackby(var/obj/item/W, var/mob/living/human/user)
	if (istype(W, /obj/item/stack))
		var/obj/item/stack/ST = W
		if (ST.amount <= 0)
			return
		else
			var/price = input(user, "What price do you want to place the [ST.amount] [ST] for sale? (dollars).") as num|null
			if (!isnum(price))
				return
			if (price <= 0)
				return
		...
		var/price = input(user, "What price do you want to place the [W] for sale? (dollars).") as num|null
		if (!isnum(price))
			return
		if (price <= 0)
			return
