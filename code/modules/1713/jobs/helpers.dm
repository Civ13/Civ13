...
/proc/total_n_of_side(x)
	...
	switch (x)
		...
		if (BLUEFACTION)
			return dead_bluefaction.len + heavily_injured_bluefaction.len + alive_bluefaction.len
		if (REDFACTION)
			return dead_redfaction.len + heavily_injured_redfaction.len + alive_redfaction.len