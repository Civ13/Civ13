...
/datum/job/vietnamese/give_random_name(var/mob/living/human/H)
	...
	H.real_name = H.name
	handle_appearence(H)
	// Including appearance change in this proc, not the best solution, but still cleaner than before

/datum/job/vietnamese/proc/handle_appearence(var/mob/living/human/H)
	var/new_hair = "Black"
...
/datum/job/american/proc/handle_appearence(var/mob/living/human/H)
	// Facial hair
	if (prob(45))
		H.f_style = pick("Selleck Mustache", "3 O'clock Shadow and Mustache","5 O'clock Shadow and Mustache","7 O'clock Shadow and Mustache","3 O'clock Shadow","5 O'clock Shadow","7 O'clock Shadow", "Elvis Sideburns","Goatee")
	else
		H.f_style = pick("Shaved")
	// Special skintone
	if (H.original_job.can_be_afro && prob(25))
...
///////////////////////////////////////
/* SOF and Insurgents used in Hostages 
=> should make a separate file for them
*/
///////////////////////////////////////
