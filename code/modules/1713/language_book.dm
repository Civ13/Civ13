...
/obj/item/weapon/language_book/attack_self(var/mob/user as mob)
  ...
  if(src.written)
    if (WWinput(user, "Do you want to learn a language from [src]?", "Learn a Language", "text", list("Yes", "No")) == "Yes")
      if(!src.lang1 in H.languages && !src.lang2 in H.languages)
      ...
      if(src.lang1 in H.languages && src.lang2 in H.languages)
        ...
        return
      var/datum/language/langtolearn = (src.lang1 in H.languages) ? src.lang2 : src.lang1
...
/obj/item/weapon/language_book/attackby(obj/item/weapon/W as obj, mob/living/carbon/human/user as mob)
  if(istype(W, /obj/item/weapon/pen))
    if(user.languages.len < 2)
      user << "<span class = 'warning'>You can't write in a language book if you only know one language!</span>"
      return
    if(written)
      user << "<span class = 'warning'>You can't write in a language book that has already been written in!</span>"
      return
