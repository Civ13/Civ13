...
/obj/item/smokerack
	name = "pop smoke rack"
	desc = "A small pod with a smoke grenade in it that will fire in a certain direction."
	icon = 'icons/obj/vehicles/vehicleparts.dmi'
	icon_state = "none"
	anchored = TRUE
	flammable = FALSE
...
/obj/item/smokerack/New()
	..()
	smoke = PoolOrNew(/datum/effect/effect/system/smoke_spread/bad)
	smoke.attach(src)

/obj/item/smokerack/Destroy()
	qdel(smoke)
	smoke = null
	return ..()

/obj/item/smokerack/attack_hand(mob/user as mob)
	if (ammo)
		ammo--
		playsound(loc, 'sound/effects/smoke.ogg', 50, TRUE, -3)
		switch (src.dir)
			if (NORTH)
				smoke.set_up(5, FALSE, locate(src.x-4,src.y,src,z))
				smoke.start()
				smoke.set_up(5, FALSE, locate(src.x+4,src.y,src,z))
				smoke.start()
			if (SOUTH)
				...
				smoke.start()
			if (EAST)
				smoke.set_up(5, FALSE, locate(src.x,src.y-4,src,z))
				smoke.start()
				smoke.set_up(5, FALSE, locate(src.x,src.y+4,src,z))
				smoke.start()
			if (WEST)
