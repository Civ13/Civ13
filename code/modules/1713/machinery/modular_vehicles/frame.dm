...
/obj/structure/vehicleparts/frame/ex_act(severity)
	switch(severity)
		...
		if (3.0)
			return
/*
/obj/structure/vehicleparts/frame/attack_hand(var/mob/living/carbon/human/H)
	if (src.loc == H.loc && (w_left[7] || w_right[7] || w_front[7] || w_back[7]))
		visible_message("[H] starts climbing out of \the [src]...")
		if (do_after(H, 40, src, can_move = FALSE))
			if (src.loc == H.loc)
				visible_message("[H] climbs out of \the [src].")
				if (w_left[7])
					switch(dir)
						if (NORTH)
							H.forceMove(locate(x-1,y,z))
						if (SOUTH)
							H.forceMove(locate(x+1,y,z))
						if (WEST)
							H.forceMove(locate(x,y-1,z))
						if (EAST)
							H.forceMove(locate(x,y+1,z))
				else if (w_right[7])
					switch(dir)
					...
					return
		if (CanPass(H,src.loc))
			visible_message("[H] starts climbing into \the [src]...")
			if (do_after(H, 40, src, can_move = FALSE))
				if (CanPass(H,src.loc))
					visible_message("[H] climbs into \the [src].")
					H.forceMove(src)
*/
