									...
									if (explosion)
										if (istype(src, /obj/structure/cannon/mortar))
											if (locate(/obj/structure/vehicleparts/frame) in target)
												...
												src.fragmentate(target)
												for (var/obj/item/projectile/bullet/pellet/fragment/F in target.contents)
													// Calculate the distance between the original turf and the fragment's turf.
													var/distance = sqrt((F.x - target.x)^2 + (F.y - target.y)^2) // Manually calculate distance
													if (distance > 2) // Adjust 10 to your desired maximum distance
														del F // Delete the fragment if it exceeds the maximum distance
