...
/datum/martial_art/wrestling/proc/check_streak(mob/living/human/A, mob/living/human/D)
	switch(streak)
		if("drop")
			streak = ""
			drop(A,D)
...
/datum/martial_art/wrestling/proc/slam(mob/living/human/A, mob/living/human/D)
	if(!D)
		return
	if(!A.pulling || A.pulling != D)
		to_chat(A, "<span class='warning'>You need to have [D] in a cinch!</span>")
	...
	if (A && D)
		...
		D.visible_message("<span class='danger'>[A] [fluff] [D]!</span>", \
						"<span class='userdanger'>You're [fluff]ed by [A]!</span>", "<span class='hear'>You hear a sickening sound of flesh hitting flesh!</span>")
	...
	D.attack_log += "\[[time_stamp()]\] <font color='orange'>Body-slammed by [A.name] ([A.ckey])</font>"
	return
...
/datum/martial_art/wrestling/proc/strike(mob/living/human/A, mob/living/human/D)
	if(!D)
		return
...
/datum/martial_art/wrestling/proc/drop(mob/living/human/A, mob/living/human/D)
	if(!D)
		return
	...
	if (A && D)
		...
		if (A)
			A.pixel_y = 0
		D.attack_log += "\[[time_stamp()]\] <font color='orange'>Legdropped by [A.name] ([A.ckey])</font>"
	return
...
/datum/martial_art/wrestling/grab_act(mob/living/human/A, mob/living/human/D)
	...
	D.visible_message("<span class='danger'>[A] gets [D] in a cinch!</span>", \
					"<span class='userdanger'>You're put into a cinch by [A]!</span>", "<span class='hear'>You hear aggressive shuffling!</span>")
	to_chat(A, "<span class='danger'>You get [D] in a cinch!</span>")
	D.Stun(rand(60,100))
	D.attack_log += "\[[time_stamp()]\] <font color='orange'>Cinched by [A.name] ([A.ckey])</font>"
	return 1
