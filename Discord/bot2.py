...
def on_message(message):
	...
	#	if message.content.startswith('!test'):
		...
		elif message.content.startswith('wryyy'):
			yield from client.send_message(message.channel, 'http://mechahitler.co.nf/wryyy.png')
		elif message.content.startswith('help'):
			yield from client.send_message(message.channel, '**List of Commands**: germanscience, germanmedicine, serverstatus, lebensraum, lebenchan, chinaman, wryyy, hdab, dabon, cpu, ping, (un)whitelistme, (un)patronme, updateserver, rebuildbinaries, host')
		...
		# patron list
			...
			if accepted:
				...
				os.system('sudo python3 /home/customer/updateserverabspaths.py')
			...
			for role in message.author.roles:
				if role.name == "Senate" or role.name == "Ahnenerbe Viceroy":
					...
					break 
			if accepted:
				os.system('sudo DreamDaemon /home/customer/LBAY/InterBay/InterBay.dmb 13007 -trusted -logself && cd &')
		...
		# elections 
			...
			if accepted:
				if message.author.name in senate_approval_for_judge_elections:
					...
					if len(senate_approval_for_judge_elections) == 1:
						yield from client.send_message(message.channel, "Judge elections have 3/3 votes and will commence. Please ensure that this bot is not shut down or restarted within the next 36 hours, or the vote will be canceled!")
						...
						# allow voting 
						judge_elections_end_at = time.time() + 129600
			...
			if len(split_message) > 1 and len(split_message[1]) > 0:
				...
				accepted = False
				for role in message.author.roles:
					if role.name in ["Staff", "Contributor", "Wiki-Helper", "Patron", "Tester", "Senate", "+1", "Voter"]:
						accepted = True 
						break
				if accepted:
					...
					yield from client.send_message(message.channel, "Rejected! To vote, you need to have one or more of the following roles: Staff, Contributor, Wiki-Helper, Patron, Tester, Senate, +1. If you're a long time player without a role, you may contact a Senator and receive the Voter role.")
			else:
			...
			if len(split_message) > 1 and len(split_message[1]) > 0:
				...
				accepted = False
				for role in message.author.roles:
					if role.name in ["Staff", "Contributor", "Wiki-Helper", "Patron", "Tester", "Senate", "+1", "Judge-Candidate"]:
						accepted = True 
						break
				if accepted:
					for role in message.author.roles:
			...
			yield from client.send_message(message.channel, "**Current election results:**")
			...
			for tuple in sorted_results:
				...
				yield from client.send_message(message.channel, "{} has {} votes, putting them in {} place.".format(candidate.capitalize(), votes, placestr))
				place += 1
...
client.run('NDQ4ODgwODE4MjExOTEzNzU4.DeclhA.a4pl-ZRCbPrmFaVnhDvV21ZcqEg')
