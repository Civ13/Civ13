...
- [x] Fix submarine start position: currently `(1000, 1000)` — should be centered at `(500, 500)`
- [x] Add torpedo items to equipment locker or map for player acquisition
- [x] Refine win/loss conditions (mission completion scoring, null-safety on `global.subcom_map.missions`)
- [x] Balance NPC weapon damage, detection ranges, and AI aggression
- [x] Crew management (track crew members, injuries, assignments, oxygen status)
- [x] Test full game loop end-to-end (map loads, process ticks run, missions generate, combat works)
- [x] Single Player mode — toggle via Server > Subcom13 tab, ghosts can interact with machinery
